...
LABEL_ASSISTANT_STORAGE_METADATA_KEY = "label_assistant_workspace_storage"
TRANSIENT_EDIT_HELPER_SUFFIXES = (
    " — editable area",
    " — editable boundary",
)
...
def save_workspace(
    ...
    *,
    ...
    for index, layer in enumerate(list(viewer.layers)):
        if is_internal_layer(layer) or _is_transient_edit_helper_name(
            getattr(layer, "name", "")
        ):
            continue
        try:
    ...
    if skipped:
        ...
        raise WorkspaceError(
            f"Project was not saved because {len(skipped)} layer(s) could not be serialized. "
            f"The existing manifest was left unchanged. {details}"
        )

    dims = getattr(viewer, "dims", None)
    selected_layer_name = (
        str(getattr(active, "name", "")) if active is not None else ""
    )
    if active is not None and _is_transient_edit_helper_name(
        selected_layer_name
    ):
        metadata = getattr(active, "metadata", {}) or {}
        selected_layer_name = (
            str(metadata.get("source_layer_name") or "")
            if isinstance(metadata, dict)
            else ""
        ) or str(_transient_edit_source_name(selected_layer_name) or "")
    payload = {
...
def load_workspace(
    ...
    *,
    ...
    """Load a project while keeping Zarr Labels writable and lazy."""
    ...
    if not isinstance(records, list) or not records:
        ...
        raise WorkspaceError(
            f"Project contains no layers and was not loaded: {path}.{suffix}"
        )
    load_records = [
        record
    ...
    viewer._label_assistant_workspace_loading = True
    try:
        if progress is not None:
            progress(0, total, "Preparing project…")
        if clear_existing:
        ...
        for index, record in enumerate(load_records):
            ...
            if progress is not None:
                progress(index, total, f"Opening layer: {name}")
            try:
                ...
                if progress is not None:
                    progress(index + 1, total, f"Processed layer: {name}")

        if progress is not None:
            progress(total, total, "Restoring viewer state…")
...
def _is_transient_edit_helper_name(name: Any) -> bool:
    """Recognize current and legacy viewport-edit helper layer names."""
    return _transient_edit_source_name(name) is not None


def _transient_edit_source_name(name: Any) -> str | None:
    normalized = str(name or "")
    if normalized.endswith("]"):
        bracket = normalized.rfind(" [")
        if bracket >= 0 and normalized[bracket + 2 : -1].isdigit():
            normalized = normalized[:bracket]
    for suffix in TRANSIENT_EDIT_HELPER_SUFFIXES:
...
def read_workspace(source: str | Path) -> tuple[Path, dict[str, Any]]:
    ...
    if payload.get("format") == LEGACY_SAM3_FORMAT:
        version = int(payload.get("version", 0))
        if version != 1:
            raise WorkspaceError(
                f"Unsupported SAM3 workspace version {version}; expected 1."
            )
        payload = dict(payload)
        payload["format"] = WORKSPACE_FORMAT
        payload["imported_from"] = "napari-sam3-workspace-v1"
        return path, payload
    if payload.get("format") != WORKSPACE_FORMAT:
        ...
        raise WorkspaceError(
            f"{path} is not a recognized Label Assistant, SAM3, or legacy "
            "shared workspace manifest."
        )
    version = int(payload.get("version", 0))
    if version != WORKSPACE_VERSION:
        raise WorkspaceError(
            f"Unsupported Label Assistant project version {version}; expected {WORKSPACE_VERSION}."
        )
    return path, payload
...
def _import_legacy_shared_workspace(
    ...
    """Map the version-2 shared/Myelin manifest to Label Assistant storage records."""
...
def create_portable_snapshot(
    ...
    *,
    ...
    portable_manifest = target_root / "project.label-assistant.json"
...
def _persist_labels_to_zarr(
    ...
    *,
    ...
    if len(shape) not in (2, 3):
        raise WorkspaceError(f"Label Assistant project Labels must be 2D or 3D; got {shape}.")
...
def _ensure_writable_zarr_layer(layer: Any, reference: dict[str, Any]) -> None:
    ...
    try:
        ...
        raise WorkspaceError(
            f"Labels store is not writable: {target}. Create a writable Label Assistant copy first."
        ) from exc
    layer.data = writable
...
def _common_record(layer: Any, layer_type: str) -> dict[str, Any]:
    ...
    if isinstance(metadata, dict) and isinstance(
        metadata.get("label_assistant_grid"), dict
    ):
        record["label_assistant_grid"] = _json_ready(
    ...
    if str(record.get("layer_type")) == "Image":
        ...
        if record.get("colormap"):
            try:
                ...
                pass
    grid = record.get("label_assistant_grid")
    if isinstance(grid, dict):
        metadata = dict(getattr(layer, "metadata", {}) or {})
        metadata["label_assistant_grid"] = dict(grid)
        try:
            layer.metadata = metadata
        except Exception:
            pass
...
def _commit_pending_local_edits(viewer: Any) -> None:
    """Move pending editable-tile pixels into source layers before saving."""
    controllers = list(
        getattr(viewer, "_label_assistant_edit_controllers", ()) or ()
    )
    for controller in controllers:
        if bool(getattr(controller, "closed", False)) or not bool(
            getattr(controller, "dirty", False)
        ):
            continue
        if bool(getattr(controller, "stroke_active", False)):
            raise WorkspaceError(
                "Finish the active paint or erase stroke before saving the "
                "project."
            )
        if not bool(controller.commit(update_status=False)):
            raise WorkspaceError(
                "Pending local-area edits could not be applied to their "
                "source Labels layer."
            )
...
def _resolve_path(value: Any, base: Path) -> Path:
    if value is None or not str(value).strip():
        raise WorkspaceError("Project storage path is missing.")
    path = Path(str(value)).expanduser()
